...
## Модуль «Автоматизация и деплой» (Airflow)

Каталог [`airflow/`](airflow/README.md) — финальный проект модуля: динамический
ETL DAG, задачи которого генерируются из Python-словаря (`CONFIG`).
Добавление нового агрегата = один новый словарь; DAG сам создаёт таблицу в БД,
идемпотентно наполняет её (`TRUNCATE + INSERT`), проверяет качество и опционально
выгружает CSV в объектное хранилище (S3/MinIO). Подробности — в
